using CutoverManager.Domain.Constants;
using CutoverManager.Domain.Entities;
using Microsoft.EntityFrameworkCore;

namespace CutoverManager.Infrastructure.Context;
...
public class AppDbContext : DbContext
{
        ...
        : base(options)
    {
        ...
        ApplyDbInitializer(this);
    }

    private void ApplyDbInitializer(AppDbContext context)
    {
        #region Áreas
        ...
        {
            if (!context.Areas.Any(q => q.Nome.Equals(area.Nome)))
                context.Add(area);
        ...
        {
            if (!context.Sistemas.Any(q => q.Nome.Equals(sistema.Nome)))
                context.Add(sistema);
        ...
        {
            if (!context.Executores.Any(q => q.Nome.Equals(executor.Nome)))
        ...
        {
            if (!context.Marcos.Any(q => q.Nome.Equals(marco.Nome)))
                context.Add(marco);
